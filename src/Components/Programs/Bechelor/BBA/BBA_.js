import React from 'react';
import './BBA_.css';

function BBA_() {
  return (
    <div>
      <div className='d-flex justify-content-center mt-5 me-5 ms-5 p-5'>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white '>
          <h2>Bachelor of Business Administration (BBA Hons)
</h2>
          <hr />
          <h4 className='paragraph'>Course Object</h4>
          <hr />
          <p className='paragraph'>
           This program is aimed at higher-caliber under graduate students who 
           are both academically able and exhibit strong managerial potential.
            This program spreads over 8 semesters and covers 132 credit hours
             in 4 years. The package will consist of 42 courses with 2 months
              internship in any reputed public or private organization.
               This program will lead to MBA 2 years program.


          </p>
        </div>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h3>Eligibility Criteria</h3>
          <hr />
          <ul>
            <li>Intermediate</li>
            <li>A level or Equivalent</li>
          </ul>
          <h4 className='paragraph '>Available Seats </h4>
          <hr />
          Only 100 seats are Available for this Program.
        </div>
      </div>
      <div className='d-flex justify-content-center me-5 ms-5 p-5'>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>1st Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>ISE-111 </td>
                <td>Islamiat/Ethics </td>
                <td>2</td>
              </tr>
              <tr>
                <td>ENG-111 </td>
                <td>English I (Language in use) </td>
                <td>3</td>
              </tr>
              <tr>
                <td>CMP-111	 </td>
                <td>Computer (Intro & App)	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-101		 </td>
                <td>Introduction to Business		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>MTH-111	 </td>
                <td>Elementary math-1		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>ECON-101		 </td>
                <td>Micro Economics		 </td>
                <td>3</td>
              </tr>
             
            </tbody>
          </table>
        </div>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>2nd Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>PST-111 </td>
                <td>Pakistan Studies </td>
                <td>2</td>
              </tr>
              <tr>
                <td>ENG-112 </td>
                <td>English II (Acadenic R & W) </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-102	 </td>
                <td>Principles of Managment		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-103		 </td>
                <td>Principles of Accounting (Basic)		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-104	 </td>
                <td> Introduction to Psychology	 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>ECON-102	 </td>
                <td> Macro Economics		 </td>
                <td>3</td>
              </tr>
             
            </tbody>
          </table>
        </div>
      </div>
      <div className='d-flex justify-content-center me-5 ms-5 p-5'>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>3rd Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>ENG-221 </td>
                <td>English III (Bus. Communication I)	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-201		 </td>
                <td>Principles of Marketing			 </td>
                <td>4</td>
              </tr>
              <tr>
                <td>BBA-202		 </td>
                <td>Financial Accounting (Basic)		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-203</td>
                <td>Cost Accounting		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-204		 </td>
                <td>Database Management System		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>STAT-211		 </td>
                <td>Elementary Stats		 </td>
                <td>3</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>4th Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>ENG-222	</td>
                <td>English IV (Bus. Communication II)	 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>SOC-211		 </td>
                <td>Introduction to Sociology		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-205		 </td>
                <td>Financial Accounting (Advance)		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-206	 </td>
                <td>Mercantail Law		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-207		</td>
                <td>Money and Banking		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-208		 </td>
                <td>Visual Programming Concept		</td>
                <td>3</td>
              </tr>
              
            </tbody>
          </table>
        </div>
      </div>
      <div className='d-flex justify-content-center me-5 ms-5 p-5'>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>5th Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>BBA-301		 </td>
                <td>Credit Management		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-302		</td>
                <td>Business Research Methods		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-303		 </td>
                <td>Company Law	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-304		</td>
                <td>Web Engineering	 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-305		 </td>
                <td> Human Resource Management		</td>
                <td>3</td>
              </tr>
              
            </tbody>
          </table>
        </div>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>6th Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>BBA-306		 </td>
                <td>Auditing		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-307		 </td>
                <td>Taxation Management (Basic)		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-308		 </td>
                <td> Management Information System		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-309		 </td>
                <td>Development Economics	 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-310		 </td>
                <td>Current Business Affairs	    	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-311		 </td>
                <td> Ordinary Differential Equations	 </td>
                <td>3</td>
              </tr>
            </tbody>
          </table>
        </div>
    </div>
     <div className='d-flex justify-content-center me-5 ms-5 p-5'>  
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>7th Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>BBA-401			 </td>
                <td>Small Business Management		</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-402			 </td>
                <td>Pakistan Economy		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-403		 </td>
                <td> Mathematics (Advanced)		 </td>
                <td>3</td>
              </tr>
              <tr>
                One Group of the following Three (Optional) 
                
              </tr>
              <tr>
                <td>BBA-404		 </td>
                <td>Managerial Accounting		    	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-405	 </td>
                <td>Financial Analysis			    	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>OR		 </td>
                
              </tr>
              <tr>
                <td>BBA-406		 </td>
                <td>Sales Management			 </td>
                <td>3</td>
              </tr>
              
              <tr>
                <td>BBA-407			 </td>
                <td>Marketing Research			 </td>
                <td>3</td>
              </tr>
              <tr>
             
                <td>OR</td>
              </tr>
              <tr>
                <td>BBA-408		 </td>
                <td>Strategic HRM			 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-409		 </td>
                <td>Organizational Development			 </td>
                <td>3</td>
              </tr>
              
             
            </tbody>
          </table>
        </div>
        <div className='shadow p-5 w-100 rounded-top ms-5 me-5 bg-white'>
          <h2>8th Semester</h2>
          <hr />
          <table className='table'>
            <tbody>
              <tr>
                <th>Course Code</th>
                <th>Course Tital</th>
                <th>Course ID</th>
              </tr>
              <tr>
                <td>BBA-410			 </td>
                <td>Statistics (Advanced)			</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-411			 </td>
                <td>E- Commerce		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-412			 </td>
                <td> Financial Institutions & Services			 </td>
                <td>3</td>
              </tr>
              <tr>
                
                <td>One Group of the following Three (Optional)</td>
              </tr>
              <tr>
                <td>BBA-413			 </td>
                <td>Financial Management		    	</td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-414		 </td>
                <td> Taxation Management		 </td>
                <td>3</td>
              </tr>
               <tr>
                
                <td>OR</td>
              </tr>
               <tr>
                <td>BBA-415			 </td>
                <td>Advertising		 </td>
                <td>3</td>
              </tr>
               <tr>
                <td>BBA-416		 </td>
                <td> Consumer Behavour	    	 </td>
                <td>3</td>
              </tr>
               <tr>
              
                <td>OR</td>
              </tr>
               <tr>
                <td>BBA-417			 </td>
                <td>Labour Laws in Pakistan		 </td>
                <td>3</td>
              </tr>
              <tr>
                <td>BBA-418		 </td>
                <td>Human Resource Development		 </td>
                <td>3</td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>    
 </div>
    
  );
}

export default BBA_;
